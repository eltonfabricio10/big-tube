...
class BigTubeWindow(Adw.ApplicationWindow):
    ...
    def setup_ui(self):
        ...
        self.preview_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)
        ...
        self.video_info_box.set_hexpand(True)
        self.preview_box.append(self.video_info_box)

        self.video_title_label = Gtk.Label()
        self.video_title_label.set_halign(Gtk.Align.START)
        ...
        self.video_details_label.set_halign(Gtk.Align.START)
        ...
        self.no_downloads_label.set_margin_top(24)
    ...
    def on_url_changed(self, entry):
        ...
        if validate_url(url) and self.settings.get('preview_thumbnail', True):
    ...
    def on_download_clicked(self, button):
        ...
        # Substitui o rótulo "Nenhum download" se necessário
        if self.no_downloads_label.get_parent() == self.downloads_list_box:
            self.downloads_list_box.remove(self.no_downloads_label)
    ...
    def on_close(self, window):
        ...
        for download in self.download_manager.active_downloads.copy():
            for row in self.downloads_list_box:
                if isinstance(row, DownloadRow) and hasattr(row, 'download_item') and row.download_item == download:
...
class HistoryWindow(Adw.ApplicationWindow):
    def __init__(self, app, messages):
        ...
        # ScrolledWindow para suportar rolagem
        scrolled = Gtk.ScrolledWindow()
        scrolled.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)

        # ListBox para conter as mensagens
        listbox = Gtk.ListBox()
        ...
        if not messages:
            messages.append("Nenhum histórico de download!")
        # Adicionar mensagens como ListBoxRow
        for msg in messages:
        ...
        # Só agora adiciona como conteúdo da janela
